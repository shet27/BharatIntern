...
async function loadEntries() {
    try {
        ...
        // Update your frontend with the entries data
        // ...
    } catch (error) {
...
function addItem() {
    ...
    fetch('http://localhost:3000/add-entry', {
            ...
            body: JSON.stringify({ type, name: name.value, amount: Number(amount.value) }),
        })
        .then(response => {
            if (response.ok) {
...
// Call loadEntries to fetch entries when the page loads
loadEntries();